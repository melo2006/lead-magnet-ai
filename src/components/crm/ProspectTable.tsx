  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between">
        <p className="text-xs text-muted-foreground">{prospects.length} prospects found</p>
      </div>
      ...
      <div className="bg-card border border-border rounded-xl overflow-hidden">
            ...
            <thead>
              <tr className="bg-secondary/50 border-b border-border text-muted-foreground uppercase tracking-wider font-semibold">
                ...
                <th className="px-3 py-2.5 text-left min-w-[200px]">
                  <SortHeader label="Business" sortKey="business_name" currentSort={sortBy} currentDir={sortDir} onSort={toggleSort} />
                </th>
                <th className="px-3 py-2.5 text-left min-w-[60px]">
                  <SortHeader label="Temp" sortKey="lead_temperature" currentSort={sortBy} currentDir={sortDir} onSort={toggleSort} />
                </th>
                <th className="px-3 py-2.5 text-left min-w-[120px]">
                  <SortHeader label="Location" sortKey="city" currentSort={sortBy} currentDir={sortDir} onSort={toggleSort} />
                </th>
                <th className="px-3 py-2.5 text-left min-w-[60px]">
                  <SortHeader label="Rating" sortKey="rating" currentSort={sortBy} currentDir={sortDir} onSort={toggleSort} />
                </th>
                <th className="px-3 py-2.5 text-left min-w-[70px]">
                  <SortHeader label="Reviews" sortKey="review_count" currentSort={sortBy} currentDir={sortDir} onSort={toggleSort} />
                </th>
                <th className="px-3 py-2.5 text-left min-w-[70px]">
                  <SortHeader label="Score" sortKey="lead_score" currentSort={sortBy} currentDir={sortDir} onSort={toggleSort} />
                </th>
                <th className="px-3 py-2.5 text-left min-w-[70px]">
                  <SortHeader label="Website" sortKey="has_website" currentSort={sortBy} currentDir={sortDir} onSort={toggleSort} />
                </th>
                <th className="px-3 py-2.5 text-left min-w-[70px]">
                  <SortHeader label="Chat" sortKey="has_chat_widget" currentSort={sortBy} currentDir={sortDir} onSort={toggleSort} />
                </th>
                <th className="px-3 py-2.5 text-left min-w-[70px]">
                  <SortHeader label="Voice AI" sortKey="has_voice_ai" currentSort={sortBy} currentDir={sortDir} onSort={toggleSort} />
                </th>
                <th className="px-3 py-2.5 text-left min-w-[75px]">
                  <SortHeader label="Booking" sortKey="has_online_booking" currentSort={sortBy} currentDir={sortDir} onSort={toggleSort} />
                </th>
                <th className="px-3 py-2.5 text-left min-w-[80px]">
                  <SortHeader label="Site Quality" sortKey="website_quality_score" currentSort={sortBy} currentDir={sortDir} onSort={toggleSort} />
                </th>
                <th className="px-3 py-2.5 text-left min-w-[55px]">
                  <SortHeader label="AI" sortKey="ai_analyzed" currentSort={sortBy} currentDir={sortDir} onSort={toggleSort} />
                </th>
              ...
              {sorted.map((p, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* Chat Widget */}
                    <td className="px-3 py-2.5">
                      <BoolBadge value={p.has_chat_widget} scanned={!!aiAnalyzed} />
                    </td>

                    {/* Voice AI */}
                    <td className="px-3 py-2.5">
                      <BoolBadge value={p.has_voice_ai} scanned={!!aiAnalyzed} />
                    </td>

                    {/* Online Booking */}
                    <td className="px-3 py-2.5">
                      <BoolBadge value={p.has_online_booking} scanned={!!aiAnalyzed} />
                    </td>
                    ...
                    <td className="px-3 py-2.5">
                      {!aiAnalyzed ? (
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          <span className={`font-medium ${qualityScore >= 70 ? "text-primary" : qualityScore >= 40 ? "text-amber-400" : "text-red-400"}`}>
                            {qualityScore}
                          </span>
                        </div>
                      )}
        ...
        {/* Expanded row detail rendered outside the table for simplicity */}
        {expandedId && (() => {
          ...
          return (
            <motion.div
              ...
              className="px-4 pb-4 pt-3 bg-secondary/20 border-t border-border/50"
            >
              {aiAnalysis && (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div>
                  <h4 className="text-xs font-semibold text-muted-foreground mb-1">Website</h4>
                  {p.website_url ? (
                    <a href={p.website_url} target="_blank" rel="noopener noreferrer" className="text-sm text-primary hover:underline truncate block">{p.website_url}</a>
                  ) : (
                    <p className="text-sm text-red-400">No website</p>
                  )}
                </div>
              </div>
