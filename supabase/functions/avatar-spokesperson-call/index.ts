...
const buildBeginMessage = (langKey: "en" | "pt" | "es", localHourRaw: unknown) => {
  ...
  if (langKey === "es") {
    return `${greeting}. Hola, ¿todo bien? Soy Aspen, de A-I Hidden Leads. ¿Sabías que el setenta y ocho por ciento de los clientes compran con el primer negocio que responde? Cada llamada perdida es dinero que le entregas a tu competidor. Te ayudamos de cinco formas: primero, un agente de voz con IA atiende tu teléfono 24/7, agenda citas y transfiere leads calientes en vivo. Segundo, el mismo chat con IA vive en tu sitio web para que ningún visitante se vaya sin respuesta. Tercero, reactivamos tu base de datos vieja con llamadas y mensajes de IA ofreciendo promociones nuevas. Cuarto, encontramos leads nuevos escaneando Google Business, reseñas y directorios locales, y mejoramos tu reputación. Y quinto, podemos poner un avatar de video con IA personalizado, como yo, en tu página principal. Prueba la simulación gratis aquí abajo: escribe tu nombre, empresa, correo y sitio web, y en menos de dos minutos verás cómo funciona en tu propio negocio. Antes de continuar, ¿cómo te llamas?`;
  }
