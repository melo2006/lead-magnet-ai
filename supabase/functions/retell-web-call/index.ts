...
const buildExactDemoOpening = ({
  ...
  return `${timeOfDayGreeting}! This is Aspen with AIHiddenLeads.com... <break time="350ms"/> hey, real quick — did you know that, statistically, the average American small business misses about 6 out of every 10 phone calls that come in? <break time="250ms"/> Six out of ten! That's more missed calls than missed Monday morning alarms... <break time="200ms"/> and honestly, each one of those is a customer who probably just called your competitor instead. <break time="350ms"/> That's literally why I exist. I answer every call, 24/7, I book appointments, I reschedule them, and when something really needs a human, I transfer the call live — warm handoff, no awkward hold music. <break time="300ms"/> So here's what we're gonna do: I'll simulate as if I was already working on the ${spokenBusinessName} website, so you can hear exactly how I'd sound for your customers. Just a quick demo, totally relaxed. <break time="400ms"/> But before we jump in — what should I call you?`;
};
